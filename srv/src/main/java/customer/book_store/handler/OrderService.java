package customer.book_store.handler;
...
public class OrderService implements EventHandler {
    ...
    public void validateBookAndDecreaseStockViaOrder(List<Orders> orders) {
    ...
    public void calculateTotal(List<Orders> orders){
        for(Orders order : orders){
            ...
            //setp2 get all item of the order
            CqnSelect setItems = Select.from(OrderItems_.class).where(i -> i.parent().ID().eq(order.getId()));
            List<OrderItems> allItems = db.run(setItems).listOf(OrderItems.class);

            //setp3 calculate and set the order total
            calculateNetAmount(allItems);

            //setp4 cal and set the orders total
